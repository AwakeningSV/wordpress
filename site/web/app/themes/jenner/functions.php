...
function bones_wpsearch($form)
{
	...
	<label class="screen-reader-text" for="s">' .
        __("Search for:", "bonestheme") .
        '</label>
	<input type="text" value="' .
        get_search_query() .
        '" name="s" id="s" placeholder="' .
        esc_attr__("Search the Site...", "bonestheme") .
        '" />
...
if (z_get_zone("home") == "") {
    global $zoninator;
    $zoninator->insert_zone("home", "Home", [
        "description" => "Content for the homepage",
    ]);
}
if (z_get_zone("leaders-senior") == "") {
    global $zoninator;
    $zoninator->insert_zone("leaders-senior", "Senior Leaders", [
        "description" => "Senior Leadership",
    ]);
}
if (z_get_zone("leaders-staff") == "") {
    global $zoninator;
    $zoninator->insert_zone("leaders-staff", "Staff Leaders", [
        "description" => "Staff Leadership",
    ]);
}
if (z_get_zone("leaders-volunteer") == "") {
    global $zoninator;
    $zoninator->insert_zone("leaders-volunteer", "Volunteer Leaders", [
        "description" => "Volunteer Leadership",
    ]);
}
if (z_get_zone("leaders-intern") == "") {
    global $zoninator;
    $zoninator->insert_zone("leaders-intern", "Intern Leaders and Proteges", [
        "description" => "Intern Leadership",
    ]);
}
if (z_get_zone("upcoming-livestream") == "") {
    global $zoninator;
    $zoninator->insert_zone("upcoming-livestream", "Upcoming Livestream", [
        "description" => "Upcoming live teaching",
    ]);
}
...
function ac_get_podcast_media_url(
    $file_prefix,
    $teaching_year,
    $teaching_month,
    $teaching_day,
...
function ac_backfill_podcast_episode($post_id, $is_video)
{
    ...
    $meta_key = $is_video ? "_video:enclosure" : "enclosure";
    ...
    if ($previous_enclosure) {
        $should_replace_existing_enclosure =
            ...
            strpos($previous_enclosure, "://media.awakeningchurch.com") !==
                false ||
        ...
        if (!$should_replace_existing_enclosure) {
            return;
        }
    } elseif ($is_video) {
    ...
    if (!$is_sunday && !$should_replace_existing_enclosure) {
        return;
    }
    ...
    $media_url = ac_get_podcast_media_url(
        $file_prefix,
        $teaching_year,
        $teaching_month,
        $teaching_day,
    ...
    if ($info["error"]) {
        ...
        $info = powerpress_get_media_info_local(
            $media_url,
            $content_type,
            0,
            ""
...
function ac_teaching_sort_filter($wp_query)
{
    if (
        ...
        $wp_query->set("order", is_tax("series") ? "ASC" : "DESC");
...
function ac_get_podcast_artwork($post)
{
    ...
    $teaching_gmt->setTimestamp($teaching_date);
    $teaching_local = new DateTime(
        $teaching_gmt->format($date_format),
        new DateTimeZone("America/Los_Angeles")
    );
    $desired_date = $teaching_local->format($date_format);

    $feed = fetch_feed(
        "https://awakeningmedia.azureedge.net/podcasts/awakening_podcast.rss"
    );
    ...
    foreach ($feed->get_items() as $item) {
        if ($enclosure = $item->get_enclosure()) {
            ...
            if (preg_match('/images\/(.*)$/', $image, $matches)) {
                $image =
                    "https://awakeningmedia.azureedge.net/podcasts/" .
                    $matches[0];
            } else {
                continue;
            }
